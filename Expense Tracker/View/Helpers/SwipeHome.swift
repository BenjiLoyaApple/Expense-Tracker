...
//  ios17
//
//  Created by Benji Loya on 30.12.2023.
//

import SwiftUI

struct SwipeHome: View {
    /// Sample Array
    @State private var colors: [Color] = [.indigo, .blue, .cyan, .teal, .mint, .green, .purple, .red, .pink, .orange,.yellow, .black, .brown, .gray]
    
    var body: some View {
        ScrollView(.vertical) {
            LazyVStack(spacing: 10) {
                ForEach(colors, id: \.self) { color in
                    SwipeAction(cornerRadius: 15, direction: color == .blue ? .leading : .trailing) {
                        CardView(color)
                    } actions: {
                        Action(tint: .blue, icon: "star.fill", isEnabled: color == .indigo) {
                            print("Bookmark")
                        }
                    ...
                    /// To Take Full Available Space
                        ...
                        .overlay {
                            GeometryReader {
                                ...
                                Color.clear
                                    .preference(key: OffsetKeySc.self, value: minX)
                                    .onPreferenceChange(OffsetKeySc.self) {
                                        scrollOffset = $0
                                    }
                            }
                        }
...
/// Offset Key
struct OffsetKeySc: PreferenceKey {
